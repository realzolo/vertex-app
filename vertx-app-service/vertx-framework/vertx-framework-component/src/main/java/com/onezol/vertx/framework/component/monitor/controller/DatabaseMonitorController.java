package com.onezol.vertx.framework.component.monitor.controller;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONArray;
import com.alibaba.fastjson2.JSONObject;
import com.onezol.vertx.framework.common.exception.ResourceNotFoundException;
...
public class DatabaseMonitorController {
    ...
    public GenericResponse<Object> getData(@PathVariable("key") String key) {
        if (!DRUID_RESOURCE_SET.contains(key) && !DRUID_PAGE_RESOURCE_SET.contains(key)) {
            throw new ResourceNotFoundException("未找到Druid资源");
        }
    ...
    private JSONArray request(String path) {
        ...
        try {
            ...
            throw new RuntimeServiceException("Druid请求失败, 请检查Druid配置服务状态");
        }
    ...
    private String requestForCookie() {
        ...
        if (response.getStatusCode() != HttpStatus.OK || isErrorMessage) {
            throw new RuntimeServiceException("Druid登录失败, 请检查Druid是否开启了Web访问权限");
        }
        if (!Objects.equals(body, "success")) {
            throw new RuntimeServiceException("Druid登录失败, 请检查用户名密码是否正确");
        }
