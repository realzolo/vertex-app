  ...
  <GiPageLayout>
    ...
    >
      ...
      <template #roleNames="{ record }">
        <GiCellTags :data="record.roles.map((item) => item.name)" />
      </template>
      <template #status="{ record }">
        <GiCellAccountStatus :status="record.status" />
      </template>
      <template #builtin="{ record }">
        <a-tag v-if="record.builtin" color="red" size="small">是</a-tag>
      ...
      <template #action="{ record }">
        <a-space>
          ...
          <a-link
            ...
            :title="record.builtin ? '系统内置数据不能删除' : '删除'"
...
const columns: TableInstance['columns'] = [
  ...
  { title: '所属部门', dataIndex: 'department', render: ({ record }) => record.department.name, minWidth: 180, ellipsis: true, tooltip: true },
  { title: '角色', dataIndex: 'roles', slotName: 'roleNames', minWidth: 165 },
