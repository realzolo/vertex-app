<template>
  ...
  >
    ...
    <template #roleNames="{ record }">
      <GiCellTags :data="record.roles.map(role => role.name)" />
    </template>
...
const columns: TableInstance['columns'] = [
  ...
  { title: '所属部门', dataIndex: 'department', render: ({ record }) => record.department.name, minWidth: 140, ellipsis: true, tooltip: true },
...
const onMulDelete = () => {
  ...
  Modal.warning({
    ...
    hideCancel: false,
    onOk: async () => {
      await unassignFromUsers(props.roleId, selectedKeys.value as number[])
...
const onDelete = (record: RoleUserResp) => {
  return handleDelete(() => unassignFromUsers(props.roleId, [record.id]), {
