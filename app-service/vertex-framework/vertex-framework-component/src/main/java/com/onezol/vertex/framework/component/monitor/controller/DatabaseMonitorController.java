...
public class DatabaseMonitorController {
    ...
    private JSONArray request(String path) {
        ...
        try {
            ...
            throw new RuntimeBizException(BizHttpStatus.INTERNAL_SERVER_ERROR, "Druid请求失败, 请检查Druid配置服务状态");
        }
    ...
    private String requestForCookie() {
        ...
        if (response.getStatusCode() != HttpStatus.OK || isErrorMessage) {
            throw new RuntimeBizException(BizHttpStatus.INTERNAL_SERVER_ERROR, "Druid登录失败, 请检查Druid是否开启了Web访问权限");
        }
        if (!Objects.equals(body, "success")) {
            throw new RuntimeBizException(BizHttpStatus.INTERNAL_SERVER_ERROR, "Druid登录失败, 请检查用户名密码是否正确");
        }
